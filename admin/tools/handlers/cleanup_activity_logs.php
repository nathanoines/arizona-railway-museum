<?php
/**
 * Handler: Cleanup Old Activity Logs
 *
 * Deletes activity log entries older than specified days.
...
try {
    ...
    if ($count === 0) {
        $_SESSION['success_message'] = 'No activity logs older than ' . $days . ' days found.';
        header('Location: /admin/tools/');
        exit;
    }
    ...
    // Log this cleanup action (will be a fresh entry)
    ...
    $log_stmt->execute([
        ...
        ':action_type' => 'cleanup_activity_logs',
        ':entity_type' => 'system',
        ':entity_id' => 0,
        ':description' => 'Deleted ' . $deleted . ' activity log entries older than ' . $days . ' days',
        ':ip_address' => $_SERVER['REMOTE_ADDR'] ?? null,
        ':user_agent' => $_SERVER['HTTP_USER_AGENT'] ?? null
    ]);

    $_SESSION['success_message'] = 'Successfully deleted ' . $deleted . ' activity log(s) older than ' . $days . ' days.';
