...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Handle flyer upload if a file was provided
    if (isset($_FILES['flyer_upload']) && $_FILES['flyer_upload']['error'] === UPLOAD_ERR_OK) {
    ...
    } elseif (isset($_FILES['flyer_upload']) && $_FILES['flyer_upload']['error'] !== UPLOAD_ERR_NO_FILE) {
    ...
    if (empty($errors)) {
        $sql = "INSERT INTO events
                    (title, event_date, event_time, description, location, is_featured, flyer_url)
                VALUES
                    (:title, :event_date, :event_time, :description, :location, :is_featured, :flyer_url)";
        ...
        try {
            ...
            $activity_stmt->execute([
                ...
                ':description' => "Added event: {$title} on " . date('M j, Y', strtotime($event_date)),
        ...
        $title = $event_date = $event_time = $description = $location = $flyer_url = '';
        $is_featured = 0;
    }
}

$page_title = 'Add Event | Arizona Railway Museum';
...
<section class="arm-hero" style="margin-top: -4.5rem; padding: 2.5rem 0 2rem;">
    <div class="grid-container">
        <div class="grid-x grid-margin-x align-middle">
            <div class="small-12 cell">
                <h1 style="margin-bottom: 0.25rem;">Add Event</h1>
                <p class="lead" style="margin-bottom: 0;">Create a new event or activity for the museum calendar.</p>
            </div>
        </div>
    </div>
</section>
    ...
    <div class="small-12 cell">
        ...
        <div class="card arm-card">
            <div class="card-section">
                ...
                <fieldset style="border: 1px solid #e0e0e0; border-radius: 8px; padding: 1rem; margin-bottom: 1rem;">
                    ...
                    <label>Link to External Flyer
                        <input type="url" name="flyer_url"
                            ...
                            value="<?php echo htmlspecialchars($flyer_url, ENT_QUOTES, 'UTF-8'); ?>">
                        <p class="help-text">Enter a URL if the flyer is hosted elsewhere</p>
                    </label>

                    <p class="help-text" style="margin-top: 0.5rem; font-style: italic;">Note: If you upload a file, it will be used instead of the URL.</p>
                </fieldset>
                ...
                <button type="submit" class="button primary expanded" style="margin-top: 1.5rem; border-radius: 8px;">Save Event</button>
                </form>
            </div>
        </div>
    </div>
